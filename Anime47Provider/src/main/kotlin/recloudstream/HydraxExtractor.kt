    ...
    private class SegmentSource(
        ...
        override fun read(sink: Buffer, byteCount: Long): Long {
            ...
            if (currentBuffer.exhausted()) {
                ...
                // QUAN TRỌNG: nếu segment này thật sự thất bại (mạng lỗi, server rate-limit khi
                // tải liên tục nhiều segment nhanh, v.v.), TUYỆT ĐỐI không được coi là "hết file"
                // (return -1L). Trước đây coi lỗi = EOF khiến downloader nghĩ đã tải xong file,
                // trong khi phần còn lại của video bị thiếu hẳn — kết quả là file có đúng dung
                // lượng khai báo (Content-Length không đổi) nhưng dữ liệu bên trong bị cắt cụt/
                // hỏng, gây lỗi khi phát dù nhìn dung lượng thì tưởng tải "thành công".
                // Nếu segIndex này là segment CUỐI CÙNG của file thật sự (đã hết dữ liệu ở phía
                // server) mới coi là EOF hợp lệ; mọi trường hợp khác mà fetch thất bại đều phải
                // ném lỗi để downloader/player biết và báo lỗi tải, thay vì âm thầm cắt ngắn.
                val segmentBytes = fetchSegmentWithRetry(segIndex)
                if (segmentBytes.isEmpty()) {
                    val isLastSegment = segStart >= totalSize
                    if (isLastSegment) return -1L
                    throw IOException(
                        "Không thể tải đoạn video (segment $segIndex) sau nhiều lần thử lại. " +
                            "File tải về có thể bị thiếu dữ liệu nếu bỏ qua lỗi này."
                    )
                }
        ...
        // Thử lại tối đa 3 lần với khoảng nghỉ ngắn trước khi coi là lỗi thật sự — giúp vượt
        // qua các lỗi mạng/rate-limit tạm thời thay vì làm hỏng cả file chỉ vì 1 segment
        // bị nghẽn tạm thời giữa hàng trăm request liên tiếp khi tải nguyên file.
        private fun fetchSegmentWithRetry(index: Int, maxAttempts: Int = 3): ByteArray {
            var lastResult = ByteArray(0)
            for (attempt in 1..maxAttempts) {
                lastResult = fetchSegment(index)
                if (lastResult.isNotEmpty()) return lastResult
